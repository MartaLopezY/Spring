	...
	CommandLineRunner initData(ProductoService productoService, CategoriaService categoriaService, UsuarioService usuarioService, ValoracionService valoracionService) {
		return args -> {
			...
			productoService.agregar(
					new CambioData(1L, "mesa", true, NORMAL, 100d,categoriaService.obtenerPorId(1L)));
			productoService.agregar(
					new CambioData(2L, "silla", false, NORMAL, 50d,categoriaService.obtenerPorId(1L)));
			productoService.agregar(
					new CambioData(3L, "estanteria", true, REDUCIDO, 70d,categoriaService.obtenerPorId(1L)));
			productoService.agregar(
					new CambioData(4L, "cojin", true, NORMAL, 20d,categoriaService.obtenerPorId(2L)));
			productoService.agregar(
					new CambioData(4L, "jarón", true, NORMAL, 5d,categoriaService.obtenerPorId(2L)));
